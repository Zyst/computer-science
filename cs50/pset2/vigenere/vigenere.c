...
bool isUpper(char);
bool isLower(char);
bool checkKeyAsAlpha(string);

int main(int argc, string argv[])
{
    // If we don't have exactly two arguments
    if (argc != 2 || !checkKeyAsAlpha(argv[1]))
    {
...
// Checks if a string has exclusively alphabetical characters
bool checkKeyAsAlpha(string key)
{
    // We start with a true check
